...
class LoanRepository(DBConnection):
    ...
    def getCustomerByID(self, cid):
        try:
            ...
            self.close()
            return False if cust is None or cust == [] else cust
    ...
    def calculateEMI(self, lid):
        try:
            ...
            emi_numerator = float(loan.principal_amount) * monthly_interest_rate * (
                pow((1 + monthly_interest_rate), loan.loan_term))
    ...
    def loanRepayment(self, lid, amount):
        try:
            ...
            if num < 1:
                ...
                self.open()
                self.c.execute(
                    f"Update Loan Set loan_term = {remaining_loan_term}, principal_amount = {remaining_loan_amount} Where loan_id = {lid}")
                self.mydb.commit()
                self.close()
